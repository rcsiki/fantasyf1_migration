 ...
 */
package com.robertcsiki.f1.web.manager;

import com.robertcsiki.f1.web.config.WebConfig;
import com.robertcsiki.f1.web.extractor.ErgastExtractor;
import com.robertcsiki.f1.web.extractor.ExtractorException;
...
import com.robertcsiki.f1.web.fc.IRuleCase;
import com.robertcsiki.f1.web.fc.ITeam;
import com.robertcsiki.f1.web.impl.Driver;
import com.robertcsiki.f1.web.impl.Player;
import com.robertcsiki.f1.web.impl.Race;
import com.robertcsiki.f1.web.impl.RacePositionDetail;
import com.robertcsiki.f1.web.impl.RaceResult;
import com.robertcsiki.f1.web.impl.Rule;
import com.robertcsiki.f1.web.impl.RuleCase;
import com.robertcsiki.f1.web.impl.Team;
	   ...
	   throws Exception	   
{
    ...
    public String drawDriverCosts() throws Exception
	{
		StringBuffer sb = new StringBuffer();
		sb.append("<table border='0' cellspacing='2' cellpadding='2'>");
	   ...
	   throws Exception
	{
		...
		for (int i = 1 ; i <= nodesCount ; i++)
		{
            ...
            IDriver driver = new Driver(strId, strName, strDescription, Integer.parseInt(strCost));
			if (replacements != null)
			{
	   ...
	   throws Exception
	{
		...
		for (int i = 1 ; i <= nodesCount ; i++)
		{
            ...
            // search for driver replacements
            NodeIterator ni1 = XPathAPI.selectNodeIterator(doc, "/teams/team[" + i
                    + "]/replacements/replacement");
            Node replacement = null;
            Node nextNode = null;
            Node firstChild = null;
            String strRaceId = null;
            String strDriverId1Repl = strDriverId1;
            String strDriverId2Repl = strDriverId2;
            Map replacements = null;
            int j = 1;
            while ((replacement = ni1.nextNode()) != null)
            {
                ...
                NodeIterator ni2 = XPathAPI.selectNodeIterator(doc, "/teams/team[" + i
                        + "]/replacements/replacement[" + j + "]/raceid");
                strRaceId = ni2.nextNode().getFirstChild().getNodeValue();
                ni2 = XPathAPI.selectNodeIterator(doc, "/teams/team[" + i
                        + "]/replacements/replacement[" + j + "]/driver1");
                if (ni2 != null && ((nextNode = ni2.nextNode()) != null)
                        && ((firstChild = nextNode.getFirstChild()) != null)) {
                    strDriverId1Repl = firstChild.getNodeValue();
                }
                ni2 = XPathAPI.selectNodeIterator(doc, "/teams/team[" + i
                        + "]/replacements/replacement[" + j + "]/driver2");
                if (ni2 != null && ((nextNode = ni2.nextNode()) != null)
                        && ((firstChild = nextNode.getFirstChild()) != null)) {
                    strDriverId2Repl = firstChild.getNodeValue();
                }
                replacements.put(strRaceId, (strDriverId1Repl + "," + strDriverId2Repl));
                j++;
            }

            ITeam team = new Team(strId, strName, strDescription, Integer.parseInt(strCost),
                    strDriverId1, strDriverId2);
            team.setReplacements(replacements);
	   ...
	   throws Exception
	{
		...
		for (int i = 1 ; i <= nodesCount ; i++)
		{
            ...
            IDriver playerDriver = getDriverById(strPlayerDriverId1);
            playerDriver.addPick();
            playerDriver.addPicker(strPlayerName);
			
			player.setDriverId2(strPlayerDriverId2);
            playerDriver = getDriverById(strPlayerDriverId2);
            playerDriver.addPick();
            playerDriver.addPicker(strPlayerName);

			player.setTeamId1(strPlayerTeamId1);
            ITeam playerTeam = getTeamById(strPlayerTeamId1);
            playerTeam.addPick();
            playerTeam.addPicker(strPlayerName);
			
			player.setTeamId2(strPlayerTeamId2);
            playerTeam = getTeamById(strPlayerTeamId2);
            playerTeam.addPick();
            playerTeam.addPicker(strPlayerName);
 ...
 */		
    ...
    private String drawStatsMostWanted(String strPlayerComponentType) throws Exception
	{		
        ...
        sb.append("<td align='center' class=\"tablelayout\">Who picked</td>");
		sb.append("</tr>");
        ...
        for (Iterator iter = list.iterator(); iter.hasNext();)
		{
			...
			{
				sb.append("<td class=\"tablelayout\">");
                sb.append(getTeamById(
                        ((IDriver) playerComponent).getTeamId(getLastDisputedRace().getId()))
                        .getName());
				sb.append("&nbsp;&nbsp;&nbsp;</td>");
			}
			else
			{
				sb.append("<td class=\"tablelayout\">");
                sb.append(getDriverById(
                        ((ITeam) playerComponent).getDriverId1(getLastDisputedRace().getId()))
                        .getName());
				sb.append("</td>");
				sb.append("<td class=\"tablelayout\">");
                sb.append(getDriverById(
                        ((ITeam) playerComponent).getDriverId2(getLastDisputedRace().getId()))
                        .getName());
				sb.append("</td>");
			}
            ...
            sb.append("</td>");
            sb.append("<td align='left' class=\"tablelayout\">");
            sb.append(playerComponent.getPickers());
			sb.append("</td></tr>");
			iPosCount ++;
		}
		sb.append("</table>");
    ...
    private String drawStatsPlayerComponentsOverall(String strPlayerComponentType) throws Exception
    {		
    	boolean bIsDriver = strPlayerComponentType.equals("drivers");
		...
		for (Iterator iter =list.iterator() ; iter.hasNext() ; )
		{
			...
			else
			{
				...
				sb.append("</td>");
				sb.append("<td align='right' class=\"tablelayout\">");
                sb.append(StringUtil.formatFloat(getDriverById(
                        ((ITeam) playerComponent).getDriverId1(getLastDisputedRace().getId()))
                        .getGrandTotalPoints()));
				sb.append("&nbsp;&nbsp;&nbsp;</td>");
				sb.append("<td class=\"tablelayout\">");
    ...
    public String getTeamIdForDriverAndRace(String driverId, String strRaceId) {
        
        ITeam team;
        for (Iterator iter = m_teams.values().iterator(); iter.hasNext();) {
            team = (ITeam) iter.next();
            if (driverId.equals(team.getDriverId1(strRaceId))
                    || driverId.equals(team.getDriverId2(strRaceId))) {
                return team.getId();
            }
        }
        return null;
