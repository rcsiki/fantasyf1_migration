...
public class RaceResult implements IRaceResult
{
    ...
    public String printOfficialResults(boolean generateCloseBttn) throws Exception
	{
		...
		for (Iterator iter = racePositionDetailsList.iterator() ; iter.hasNext() ; )
		{
			...
			sb.append("</td>");
			sb.append("<td class='tablelayout' align='left'>");
            sb.append(cm.getTeamById(
                    cm.getDriverById(raceResultDetail.getDriverId()).getTeamId(race.getId()))
                    .getName());
			sb.append("</td>");
			sb.append("<td class='tablelayout' align='right'>");
